...
const PetHealthRecord = ({ petId }) => {
    ...
    const fetchRecords = async () => {
        try {
            // Check localStorage first for demo purposes if backend isn't fully ready
            const localRecords = JSON.parse(localStorage.getItem(`health_records_${actualPetId}`)) || [];
            ...
            // Don't show error toast on load to avoid spamming if API is missing
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleAddRecord = async (e) => {
        ...
        try {
            ...
            // await api.post(`/pets/${actualPetId}/health-records`, recordToAdd);

            // For now, save to local storage to demonstrate functionality
            const updatedRecords = [recordToAdd, ...records];
            setRecords(updatedRecords);
            localStorage.setItem(`health_records_${actualPetId}`, JSON.stringify(updatedRecords));
    ...
    const canEdit = user?.role === 'NGO' || user?.role === 'Owner';
